...
"""Runs all tagged Bazel synthesis targets and regenerates PPA.md."""
...
def render_markdown(metrics: Iterable[PpaMetrics]) -> str:
...
def query_targets(
    workspace: Path, bazel: str, output_base: Optional[Path]
) -> list[str]:
    proc = subprocess.run(
        _bazel_command(
            bazel,
            output_base,
            "query",
            'attr(tags, "ppa-synth", //...) except attr(tags, "ppa-synth-sandbox", //...)',
            "--output=label",
        ),
        cwd=workspace,
        check=False,
...
def main() -> int:
    ...
    parser.add_argument(
        "--check", action="store_true", help="Fail instead of updating a stale PPA.md."
    )
    parser.add_argument(
    ...
    targets = (
        sorted(args.target)
        if args.target
        else query_targets(workspace, args.bazel, bazel_output_base)
    )
    if not targets:
        raise SystemExit("no ppa-synth targets found")
