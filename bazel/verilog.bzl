...
def verilog_elab_and_lint_test_suite(name, params, **kwargs):
    ...
    to the base name, followed by the parameter values.
    """
    param_keys = sorted(params.keys())
    param_values_list = [params[key] for key in param_keys]
    combinations = _cartesian_product(param_values_list)

    # Create a verilog_elab_test for each combination of parameters
    for combination in combinations:
        params = dict(zip(param_keys, combination))
        verilog_elab_test(
            name = _make_test_name(name + "_elab", param_keys, combination),
            params = params,
            **kwargs
        )
        verilog_lint_test(
            name = _make_test_name(name + "_lint", param_keys, combination),
            params = params,
            **kwargs
        )
