...
"""
This script is used to generate the shared FIFO Verilog code.
It uses Jinja2 templates to generate the code.
The templates are stored in the fifo/rtl/fifo_codegen directory.
The script takes the following arguments:
- --shared: Generate shared FIFO or single-flow FIFO. (currently only shared is supported)
- --sharing-type: The type of sharing to use for the FIFO controller, either "dynamic" or "pseudo-static".
- --push-intf: The interface type for the push side, either "credit" or "ready".
- --pop-intf: The interface type for the pop side, either "credit" or "ready".
- --ext-arbiter: Generate with external arbiter interface.
- --ctrl-only: Generate only the controller module. If false, generate FIFO with flop-based storage.
- --template: The path to the Jinja2 template file.
- --output-file: The file to write the generated Verilog code to.
"""
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The file to write the generated Verilog code to.",
    )
