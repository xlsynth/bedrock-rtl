...
`endif
  ...
  task automatic drive_push(input logic [Width-1:0] data);
    ...
    int timeout;  // Remaining cycles before reporting a push timeout.
    ...
    do begin
      @(posedge clk);
      timeout--;
    end while ((timeout > 0) && !(push_valid && push_ready));

    td.check(timeout > 0, "push timed out");
    @(negedge clk);
    push_valid = 1'b0;
    push_data  = '0;
  endtask
  ...
  task automatic drive_pop(output logic [Width-1:0] data);
    ...
    int timeout;  // Remaining cycles before reporting a pop timeout.
    ...
    do begin
      @(posedge clk);
      timeout--;
    end while ((timeout > 0) && !(pop_ready && pop_valid));

    td.check(timeout > 0, "pop timed out");
    data = pop_data;
    @(negedge clk);
    pop_ready = 1'b0;
  endtask
  ...
  task automatic wait_for_pop_valid();
    ...
    int timeout;  // Remaining cycles before reporting a wait timeout.

    timeout = TimeoutCycles;
    while ((timeout > 0) && !pop_valid) begin
      sample_idle_cycle();
      timeout--;
    end
    td.check(timeout > 0, "pop_valid timed out");
  endtask

  task automatic drain_all();
    // Intent: Keep popping until all scoreboarded items are observed or timeout expires.
    logic [Width-1:0] drained_data;  // Payload returned by each drive_pop transaction.

    while (expected_count > 0) begin
      drive_pop(drained_data);
    end
  endtask
