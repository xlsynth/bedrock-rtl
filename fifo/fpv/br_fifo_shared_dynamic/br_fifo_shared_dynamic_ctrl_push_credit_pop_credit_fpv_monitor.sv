...
// FPV monitor for br_fifo_shared_dynamic_ctrl_push_credit_pop_credit.
...
module br_fifo_shared_dynamic_ctrl_push_credit_pop_credit_fpv_monitor #(
  ...
  `BR_ASSERT_CR(all_pushes_written_a, data_ram_wr_valid == push_valid, clk, fv_rst)
  for (genvar p = 0; p < NumWritePorts; p++) begin : gen_write_data
    `BR_ASSERT_CR(push_data_written_a, push_valid[p] |-> data_ram_wr_data[p] == push_data[p], clk,
                  fv_rst)
  end

endmodule : br_fifo_shared_dynamic_ctrl_push_credit_pop_credit_fpv_monitor

bind br_fifo_shared_dynamic_ctrl_push_credit_pop_credit
    br_fifo_shared_dynamic_ctrl_push_credit_pop_credit_fpv_monitor #(
    .NumWritePorts(NumWritePorts),
    .NumReadPorts(NumReadPorts),
    .NumFifos(NumFifos),
    .Depth(Depth),
