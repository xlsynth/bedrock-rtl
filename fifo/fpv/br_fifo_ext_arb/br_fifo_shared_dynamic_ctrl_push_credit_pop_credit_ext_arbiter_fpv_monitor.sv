...
module br_fifo_shared_dynamic_ctrl_push_credit_pop_credit_ext_arbiter_fpv_monitor #(
  ...
  ext_arb_fv_monitor #(
      ...
      // arb_request comes from the credit-gated read join. Dynamic withholding
      // can withdraw eligibility before a delayed external grant, so this DUT
      // does not promise request hold-until-grant.
      .AssertRequestHeldUntilGrant(0)
  ) fv_ext_arb (
      .clk,
      .rst(fv_rst),
      .arb_request,
