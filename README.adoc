...
a| Concurrent integration assertion that an expression is known (not X), with implicit `clk` and `rst` names.
Disable by defining `BR_DISABLE_INTG_CHECKS`.
| Yes
| Yes

| `BR_ASSERT_KNOWN_VALID_INTG`
a| Concurrent integration assertion that an expression is known (not X) when a valid condition is true, with implicit `clk` and `rst` names.
Disable by defining `BR_DISABLE_INTG_CHECKS`.
| Yes
| Yes

| `BR_ASSERT_KNOWN_CR_INTG`
a| Concurrent integration assertion that an expression is known (not X), with explicit clock and reset names.
Disable by defining `BR_DISABLE_INTG_CHECKS`.
| Yes
| Yes

| `BR_ASSERT_KNOWN_VALID_CR_INTG`
a| Concurrent integration assertion that an expression is known (not X) when a valid condition is true, with explicit clock and reset names.
Disable by defining `BR_DISABLE_INTG_CHECKS`.
| Yes
| Yes
