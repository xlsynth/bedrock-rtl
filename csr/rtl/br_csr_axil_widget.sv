...
// Bedrock-RTL AXI4-Lite to CSR Widget
//
// Converts an AXI4-Lite interface to the Simple CSR Bus (SCB) interface.
//
...
//
// The SCB interface will not always guarantee that requests will make forward progress.
// A timeout and abort mechanism is provided to handle a SCB peripheral that is unresponsive.
//
...
// When the watchdog timer expires, the abort signal is asserted for a single cycle.
// The timer is then reset and counts for another timeout period. If no response is received before the
...
module br_csr_axil_widget #(
    ...
    output logic [StrobeWidth-1:0] csr_req_strb,
  ...
  // Instruction access through SCB is not supported
  `BR_ASSERT_INTG(legal_axil_awprot_a, axil_awvalid |-> axil_awprot[2] == 1'b0)
  `BR_ASSERT_INTG(legal_axil_arprot_a, axil_arvalid |-> axil_arprot[2] == 1'b0)
  ...
  assign csr_read_req = '{
          ...
          strb: {StrobeWidth{1'b0}},
          write: 1'b0,
          secure: !axil_arprot[1],
          privileged: axil_arprot[0]
      };

  // SCB protocol doesn't support instruction access
  ...
  assign csr_req_strb = csr_req.strb;
