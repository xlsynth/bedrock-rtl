...
// Bedrock-RTL Weighted Least-Recently-Used Flow Mux Testbench

module br_flow_mux_weighted_lru_tb;
  ...
  int unsigned lru_priority[NumFlows];

  br_flow_mux_weighted_lru #(
      .NumFlows(NumFlows),
      .Width(Width),
      .MaxWeight(MaxWeight),
  ...
  initial begin
    ...
    for (int i = 0; i < (2 * NumFlows); i++) begin
      drive_and_check(valid, weight, 1'b1, $sformatf("equal weights LRU cycle %0d", i));
    end
    ...
    drive_and_check(valid, weight, 1'b0, "backpressure holds weighted LRU priority");
    drive_and_check(valid, weight, 1'b1, "weighted LRU priority resumes after backpressure");
...
endmodule : br_flow_mux_weighted_lru_tb
