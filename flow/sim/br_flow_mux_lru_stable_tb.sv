...
// Bedrock-RTL Stable Least-Recently-Used Flow Mux Testbench

module br_flow_mux_lru_stable_tb;
  ...
  initial begin
    ...
    @(posedge clk);
    #1;
    check_output(8'h21, "flow one should become least recently used");
    ...
    @(posedge clk);
    #1;
    check_output(8'h32, "flow two should become least recently used");

    @(negedge clk);
    push_valid[2] = 1'b0;
    @(posedge clk);
    #1;
    check_output(8'h10, "flow zero should become least recently used again");
...
endmodule : br_flow_mux_lru_stable_tb
