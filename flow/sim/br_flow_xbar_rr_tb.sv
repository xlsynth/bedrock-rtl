...
// Bedrock-RTL Round-Robin Flow Crossbar Testbench

module br_flow_xbar_rr_tb;
  ...
  initial begin
    ...
    push_valid   = 3'b111;
    #1;
    check_contention(3'b001, "initial round-robin contention");

    @(posedge clk);
    #1;
    check_contention(3'b010, "round-robin should rotate to flow one");

    @(negedge clk);
    pop_ready[1] = 1'b0;
    #1;
    check_accept('0, "stalled round-robin contention");
    check_pop(2'b10, 1, push_data[1], "stalled round-robin output");

    @(posedge clk);
    @(negedge clk);
    pop_ready[1] = 1'b1;
    #1;
    check_contention(3'b010, "round-robin priority should hold through a stall");

    @(posedge clk);
    #1;
    check_contention(3'b100, "round-robin should rotate to flow two");

    @(posedge clk);
    #1;
    check_contention(3'b001, "round-robin should wrap to flow zero");

    @(posedge clk);
    @(negedge clk);
    push_valid[0] = 1'b0;
    #1;
    check_contention(3'b010, "round-robin should drain flow one next");

    @(posedge clk);
    @(negedge clk);
    push_valid[1] = 1'b0;
    #1;
    check_contention(3'b100, "round-robin should drain flow two last");
