...
// Bedrock-RTL Fixed-Priority Flow Crossbar Testbench

module br_flow_xbar_fixed_tb;
  ...
  initial begin
    ...
    push_valid   = 3'b111;
    #1;
    check_contention(3'b001, "initial fixed-priority contention");

    @(posedge clk);
    @(negedge clk);
    pop_ready[1] = 1'b0;
    #1;
    check_accept('0, "stalled fixed-priority contention");
    check_pop(2'b10, 1, push_data[0], "stalled fixed-priority output");

    @(posedge clk);
    @(negedge clk);
    pop_ready[1] = 1'b1;
    #1;
    check_contention(3'b001, "fixed priority should hold through a stall");

    @(posedge clk);
    @(negedge clk);
    push_valid[0] = 1'b0;
    #1;
    check_contention(3'b010, "fixed priority should drain flow one next");

    @(posedge clk);
    @(negedge clk);
    push_valid[1] = 1'b0;
    #1;
    check_contention(3'b100, "fixed priority should drain flow two last");
...
endmodule : br_flow_xbar_fixed_tb
