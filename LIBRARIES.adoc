...
| Routes CSR requests using an externally supplied one-hot downstream selection.
| Elab/lint, FPV.
...
| `br_flow_buffer`
| Selects a ready/valid buffer implementation for a configurable storage depth.
| Elab/lint, Sim.
...
| `br_flow_valve`
| Gates data-less ready/valid flows with per-flow enable controls.
| Elab/lint, Sim.
