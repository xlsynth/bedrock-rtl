...
import ecc.rtl.br_ecc_secded_encoder;
import ecc.rtl.br_ecc_secded_decoder;
...
fn quickcheck_no_errors(message: bits[K]) -> bool {
    let codeword = br_ecc_secded_encoder_xls(message);
    let (decoded_message, ce, due) = br_ecc_secded_decoder_xls(codeword);
...
fn quickcheck_single_bit_error_correction(message: bits[K], pos: u32) -> bool {
    ...
    if pos < N {
        ...
        let (decoded_message, ce, due) = br_ecc_secded_decoder_xls(received_codeword);
...
fn quickcheck_double_bit_error_detection(message: bits[K], pos0: u32, pos1: u32) -> bool {
    ...
    if pos0 < N && pos1 < N && pos0 < pos1 {
        ...
        let (_, ce, due) = br_ecc_secded_decoder_xls(received_codeword);
        !ce && due
    } else {
        true
    }
